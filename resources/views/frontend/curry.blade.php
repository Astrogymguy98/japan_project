<x-fronted-layout>
    ...
    <section>
        <div class="container m-auto py-10 grid md:grid-cols-2 gap-10">
            <div class="relative">
                <img src="{{ asset(Storage::url($curry->image)) }}" alt="">
                @if ($curry->discount > 0)
                    <span class="absolute top-0 right-0 bg-[red]/20 text-[red] px-3 py-1">{{ $curry->discount }}%
                        off</span>
            </div>
            @endif
            <div class="">
                ...
                <div>
                    <span class="text-2xl">Rs.{{ $curry->price - ($curry->discount * $curry->price) / 100 }}</span>
                    @if ($curry->discount > 0)
                        <span class=" text-sm line-through text-[red]">Rs.{{ $curry->price }}</span>
                    @endif
                </div>

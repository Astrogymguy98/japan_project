<x-frontend-layout>
    ...
    <section>
        <div class="container m-auto py-10 grid md:grid-cols-2 gap-10">
            <div class="relative">
                <img src="{{ asset(Storage::url($naan->image)) }}" alt="">
                @if ($naan->discount > 0)
                    <span class="absolute top-0 right-0 bg-[red]/20 text-[red] px-3 py-1">{{ $naan->discount }}%
                        off</span>
            </div>
            @endif
            <div class="">
                ...
                <div>
                    <span class="text-2xl">Rs.{{ $naan->price - ($naan->discount * $naan->price) / 100 }}</span>
                    @if ($naan->discount > 0)
                        <span class=" text-sm line-through text-[red]">Rs.{{ $naan->price }}</span>
                    @endif
                </div>
